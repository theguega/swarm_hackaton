...
class SwarmSearchMoveBlock:
    def __init__(self, n_robots=4, Lx=5.0, Ly=5.0):
        rospy.init_node("swarm_search_move_block", anonymous=True)
        ...
        self.phase = "search"  # search → surround → test → move → done
        self.leader_id = None
        ...
        rospy.sleep(1.0)
        rospy.loginfo(f"[Swarm] {n_robots} robots initialized.")
    ...
    def move_toward(self, idx, goal, speed_scale=1.0):
        ...
        if abs(heading_error) > 0.2:
            ...
            cmd.linear.x = self.speed * speed_scale
        ...
        if self.phase == "search":
            for i in range(self.n_robots):
                if self.indices[i] < len(self.paths[i]):
                    ...
                    # 🔍 Simulated detection
                    if random.random() < 0.0008 and self.leader_id is None:
                        tx, ty = goal
                        rospy.loginfo(
                            f"🎯 Robot {i} detected target at ({tx:.2f}, {ty:.2f})"
                        )
                        self.leader_id = i
                        self.block_position = np.array([tx, ty])
            ...
            if done:
                rospy.loginfo("🤖 All robots surrounded the block. Testing movement...")
                self.phase = "test"

        elif self.phase == "test":
            # For now assume test success (you can later use laser/contact sensor)
            rospy.sleep(1.0)
            rospy.loginfo(
                "✅ Block confirmed movable. Initiating collective transport."
            )
            self.move_direction = self.home_position - self.block_position
            self.move_direction /= np.linalg.norm(self.move_direction)
            self.phase = "move"

        elif self.phase == "move":
            # Each robot keeps formation, all move together
            move_speed = 0.08
            done = True
            for i in range(self.n_robots):
                goal = self.surround_positions[i] + self.move_direction * move_speed
                self.move_toward(i, goal, speed_scale=0.5)
                self.surround_positions[i] = goal  # update dynamic goal
            # Update block position (simulate)
            self.block_position += self.move_direction * move_speed

            # Check if home reached
            if np.linalg.norm(self.block_position - self.home_position) < 0.2:
                rospy.loginfo("🏁 Block delivered to home position. Stopping swarm.")
                self.phase = "done"

    def run(self):
        rospy.loginfo("[Swarm] Running search-surround-move controller...")
...
if __name__ == "__main__":
    try:
        node = SwarmSearchMoveBlock(n_robots=4)
        node.run()
